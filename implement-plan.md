...
- Start implementing only after creating tests that specify the expected behavior (TDD-first). You must:
...
Use your tools to investigate - mainly for targeted debugging or exploring unfamiliar territory.
...
If you encounter a situation where you need to rollback or recover:

- **Identify the issue**: Clearly define what went wrong.
- **Assess the impact**: How much is broken? Can you fix it?
- **Rollback**: If possible, revert your code changes using `git checkout` or similar.
- **Recover**: If data was lost or corrupted, attempt to recover it.
- **Report**: If you cannot fix it, report the issue clearly to the user and ask for guidance.

If the plan involves migrations or data changes, be extra careful. If you are unsure, STOP and ask for guidance before making changes that might be hard to reverse.
